...
describe('Ed25519PrivateKey', () => {
    ...
    it('derive() produces correct value', async () => {
        ...
        const key3 = await Ed25519PrivateKey.fromMnemonic(iosWalletMnemonic);
        const key4 = key3.derive({ account: 0, index: 0, change: 0 });
        const expectedBytes = Uint8Array.from(Buffer.from(iosWalletPrivKey, 'hex'));
        expect(key4.toBytes()).toStrictEqual(expectedBytes.subarray(0, 32));
        expect(key4.publicKey.toBytes()).toStrictEqual(expectedBytes.subarray(32));
    });
});
