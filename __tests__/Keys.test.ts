...
describe('Ed25519PrivateKey', () => {
    ...
    it('derive() produces correct value', async () => {
        const key1 = await Ed25519PrivateKey.fromMnemonic(iosWalletMnemonic);
        const key2 = key1.derive(0);
        const expectedBytes = Uint8Array.from(Buffer.from(iosWalletPrivKey, 'hex'));
        expect(key2.toBytes()).toStrictEqual(expectedBytes.subarray(0, 32));
        expect(key2.publicKey.toBytes()).toStrictEqual(expectedBytes.subarray(32));
    });
});
