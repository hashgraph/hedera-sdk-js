...
describe("AccountCreate", function () {
    ...
    it("should create account with a single key passed to `KeyList`", async () => {
        const env = await IntegrationTestEnv.new();
        ...
        const txAccountCreateReceipt = await txAccountCreate.getReceipt(env.client);
        ...
        expect(info.key.toArray()[0].toString()).to.be.equal(publicKey.toString());
    });
