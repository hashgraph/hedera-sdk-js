...
import ContractUpdateTransaction from "../src/contract/ContractUpdateTranscation";
...
describe("ContractUpdate", function () {
    it("should be executable", async function () {
        ...
        expect(info.contractId).to.be.equal(contract);
        expect(info.accountId).to.be.not.null;
        expect(
            info.accountId != null ? info.accountId.toString() : ""
        ).to.be.equal(contract.toString());
        expect(info.adminKey).to.be.not.null;
        expect(
            info.adminKey != null ? info.accountId.toString() : ""
        ).to.be.equal(operatorKey.toString());
        expect(info.storage).to.be.equal(926);
