...
import compiledContract from "./resources/ReturnDataTypes.json" assert { type: "json" };
import setDataContract from "./resources/SetData.json" assert { type: "json" };
...
let setDataBytecode = setDataContract.bytecode;
describe("ContractFunctionParameters", function () {
    ...
    before(async function () {
        ...
        //////////////////// Deploy the SET DATA CONTRACT ////////////////////

        const fileCreateTx2 = new FileCreateTransaction()
            .setKeys([env.operatorKey])
            .freezeWith(env.client);
        const fileCreateSign2 = await fileCreateTx2.sign(env.operatorKey);
        const fileCreateSubmit2 = await fileCreateSign2.execute(env.client);
        const fileCreateRx2 = await fileCreateSubmit2.getReceipt(env.client);
        const bytecodeFileId2 = fileCreateRx2.fileId;
        console.log(`- The bytecode file ID is: ${bytecodeFileId2} \n`);

        //Append contents to the file
        const fileAppendTx2 = new FileAppendTransaction()
            .setFileId(bytecodeFileId2)
            .setContents(setDataBytecode)
            .setMaxChunks(10)
            .freezeWith(env.client);
        const fileAppendSign2 = await fileAppendTx2.sign(env.operatorKey);
        const fileAppendSubmit2 = await fileAppendSign2.execute(env.client);
        const fileAppendRx2 = await fileAppendSubmit2.getReceipt(env.client);
        console.log(
            "Status of file append is",
            fileAppendRx2.status.toString()
        );

        // Instantiate the contract instance
        const contractTx2 = await new ContractCreateTransaction()
            //Set the file ID of the Hedera file storing the bytecode
        ...
        //Submit the transaction to the Hedera test network
        const contractResponse2 = await contractTx2.execute(env.client);

        //Get the receipt of the file create transaction
        const contractReceipt2 = await contractResponse2.getReceipt(env.client);

        //Get the smart contract ID
        newContractId2 = contractReceipt2.contractId;

        //Log the smart contract ID
        console.log("The smart contract ID is " + newContractId2);
    });
    ...
    it("should return the right max int24 value", async function () {
        const contractQuery = await new ContractCallQuery()
            ...
            .setContractId(newContractId)
            //Set the contract function to call
            .setFunction("setMessage", ["New message"])
            //Set the query payment for the node returning the request
            //This value must cover the cost of the request otherwise will fail
            .setQueryPayment(new Hbar(10));
    ...
    it.only("test the new class Contract", async function () {
        const contractInstance = await new NewContractExecuteTransaction()
            //Set the gas for the query
            .setGas(15000000)
            //Set the contract ID to return the request for
            .setContractId(newContractId2)
        ...
        //Submit to a Hedera network
        const txResponse = await contractInstance.execute(env.client);

        const receipt = await txResponse.getReceipt(env.client);
        console.log("HERE receipt", receipt);
        ...
        //Submit to a Hedera network
        const txResponse2 = await contractQuery.execute(env.client);
        const result = txResponse2.getResult(["string"]);
        console.log("HERE Responce:", result);
        ...
        const contractQuery2 = await new ContractCallQuery()
            ...
            .setContractId(newContractId2)
            //Set the contract function to call
            .setFunction("second_message")
            //Set the query payment for the node returning the request
            //This value must cover the cost of the request otherwise will fail
            .setQueryPayment(new Hbar(10));

        //Submit to a Hedera network
        const txResponse22 = await contractQuery2.execute(env.client);
        const result2 = txResponse22.getResult(["string"]);
        console.log("HERE Responce2", result2);
    });
