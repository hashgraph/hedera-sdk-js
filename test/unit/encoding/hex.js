...
describe("hex", () => {
    ...
    it("should stringify", () => {
        expect(hex.stringify(bytes1)).to.equal(hex1);
        expect(hex.stringify(bytes2)).to.equal(hex2);
    });

    it("should parse", () => {
        expect(hex.parse(hex1)).to.deep.equal(bytes1);
        expect(hex.parse(hex2)).to.deep.equal(bytes2);
    });
});
