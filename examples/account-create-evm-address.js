...
async function main() {
    ...
    try {
        ...
        const mirrorNodeEvmAddress =
            mirrorNodeAccountInfo.evm_address.substring(2); // eslint-disable-line @typescript-eslint/no-unsafe-member-access,@typescript-eslint/no-unsafe-call
        ...
        evmAddress === accountInfoEvmAddress
            ? console.log(
                  `The evm address provided for the account matches the one in the mirror node and the one from 'AccountInfoQuery'`
              )
