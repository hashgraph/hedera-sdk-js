...
async function main() {
    ...
    /**
     *
     * Step 7: Remove the signature for user 1 from the transaction
     *
     **/
    ...
    console.log("\nREMOVED user signatures below: \n");

    if (Array.isArray(user1Signatures) && Array.isArray(user2Signatures)) {
        console.log(
            "removeUser1Signatures =>",
            removeUser1Signatures.map((user1Sig) =>
                PrivateKey.fromBytes(user1Sig).toStringDer(),
            ),
        );
    }

    const signaturesInTheTransaction2 =
        getAllSignaturesFromTransaction(signedTransaction);

    console.log("\n\nSignatures left in the transaction: ");
    console.log(signaturesInTheTransaction2);
...
const getAllSignaturesFromTransaction = (signedTransaction) => {
    ...
    for (const transaction of signedTransaction._signedTransactions.list) {
        if (transaction.sigMap && transaction.sigMap.sigPair) {
            for (const sigPair of transaction.sigMap.sigPair) {
                if (sigPair.ed25519) {
                    signaturesInTheTransaction2.push(
    ...
    return signaturesInTheTransaction2;
};
